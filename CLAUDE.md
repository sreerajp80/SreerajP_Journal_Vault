...
Claude Code loads this file at the start of every session in this repository.
Read it before making any change. This is a **Thin** file: it holds identity, commands and rule
summaries, and points to `docs/` for the detail. `AGENTS.md` carries the same rules for other AI
tools — if you change a rule here, change it there too.
...
- Keystore lives at `android/<name>.jks`; `android/key.properties` points to it. Both are
  git-ignored and must never be committed. Format:
...
AGENTS.md            # the same rules for other AI agents and LLMs

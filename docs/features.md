...
- **Unified Settings Screen**:
  - The Settings tab (`_SettingsTab`) is a menu of five cards — Security, Appearance, Storage, Permissions and About. Each card shows an icon, the section name and a one-line description, and opens a page holding only that section's rows.
  - Security page (`_SecuritySettingsScreen`): lock mode, auto-lock timeout, attachment-level lock, screenshot blocking, security events, and two "Coming soon" placeholders for Tamper Alerts and — when sync UI is disabled — Sync Conflicts. Switching lock mode closes this page, because the switch locks the app straight away.
  - Appearance page (`_AppearanceSettingsScreen`): theme choice.
  - Storage page (`_StorageSettingsScreen`): attachment location, migration, storage usage, backup health, import, and — when sync UI is disabled — a "Coming soon" Sync Health placeholder.
  - Permissions page (`_PermissionsSettingsScreen`): the permissions status rows.
  - The About card opens the existing `AboutScreen` directly.
